            ...
            <div>
                <form method="POST" action="{{ route('education.store', ['id' => Auth::user()->id] ) }}" enctype="multipart/form-data">
                    ...
                    <div class="m-3">
                        ...
                        <x-text-input id="country_of_college" name="country_of_college" type="text" class="mt-1 block w-2/5" required autofocus autocomplete="skill" />
                        <x-input-error class="mt-2" :messages="$errors->get('country_of_college')" />
                    </div>

                    <div class="m-3">
                        <x-input-label for="title" :value="__('Title')"/>
                        <x-text-input id="title" name="title" type="text" class="mt-1 block w-2/5" required autofocus autocomplete="title" />
                        <x-input-error class="mt-2" :messages="$errors->get('title')" />
                    </div>

                    <div class="m-3">
                        <x-input-label for="major" :value="__('Major')"/>
                        <x-text-input id="major" name="major" type="text" class="mt-1 block w-2/5" required autofocus autocomplete="major" />
